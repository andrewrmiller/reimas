/**
 * Enumeration of all HTTP request methods.
 */
export enum HttpMethod {
   ...
   * request can be done before deciding to download a large resource to save
   ...
   *
   * The HTTP CONNECT method method starts two-way communications with the
   ...
   *
   * The HTTP OPTIONS method is used to describe the communication options for
   * the target resource. The client can specify a URL for the OPTIONS method,
...
/**
 * Enum of all RFC7231 HTTP Status Codes.
 *
 * Sourced status codes from https://github.com/prettymuchbryce/node-http-status
 * and converted to an enumeration for easier usage.
 */
export enum HttpStatusCode {
   ...
   * body. TRACE: The message body contains the request message as received by
   * the server
   */
  OK = 200,
